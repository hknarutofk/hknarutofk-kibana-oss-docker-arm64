...
function query(req, panel, annotation, esQueryConfig, indexPattern, capabilities) {
  return next => doc => {
    ...
    if (annotation.query_string) {
      doc.query.bool.must.push((0, _esQuery.buildEsQuery)(indexPattern, [annotation.query_string], [], esQueryConfig));
    }

    if (!annotation.ignore_panel_filters && panel.filter) {
      doc.query.bool.must.push((0, _esQuery.buildEsQuery)(indexPattern, [panel.filter], [], esQueryConfig));
    }
