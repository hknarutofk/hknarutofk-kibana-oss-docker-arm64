...
async function manageUuid(server) {
  const config = server.config();
  const fileName = 'uuid';
  const uuidFile = (0, _path.join)(config.get('path.data'), fileName);

  async function detectUuid() {
    const readFile = _bluebird.default.promisify(_fs.readFile);
    ...
    try {
      ...
      server.log(['error', 'read-uuid'], err); // Note: this will most likely be logged as an Unhandled Rejection
  ...
  async function writeUuid(uuid) {
    const writeFile = _bluebird.default.promisify(_fs.writeFile);

    try {
      return await writeFile(uuidFile, uuid, {
        encoding: FILE_ENCODING
      });
    } catch (err) {
      server.log(['error', 'write-uuid'], err); // Note: this will most likely be logged as an Unhandled Rejection
  ...
  if (!serverConfigUuid) {
    // config uuid missing
    serverConfigUuid = _uuid.default.v4();
    config.set('server.uuid', serverConfigUuid);
  }
