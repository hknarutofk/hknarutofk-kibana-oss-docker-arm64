...
var _is_es_compatible_with_kibana = _interopRequireDefault(require("./is_es_compatible_with_kibana"));
...
function ensureEsVersion(server, kibanaVersion) {
  ...
  return callWithInternalUser('nodes.info', {
    ...
    const incompatibleNodes = []; // Aggregate ES nodes which should prompt a Kibana upgrade.

    const warningNodes = [];
    (0, _lodash.forEach)(info.nodes, esNode => {
      if (!(0, _is_es_compatible_with_kibana.default)(esNode.version, kibanaVersion)) {
