...
var _abortable_request_handler = require("./abortable_request_handler");
...
function createProxy(server) {
  ...
  server.route({
    ...
    handler: (0, _abortable_request_handler.abortableRequestHandler)((signal, req, h) => {
      const {
        query,
        payload
      } = req;
  ...
  server.route({
    ...
    handler: (0, _abortable_request_handler.abortableRequestHandler)(async (signal, req) => {
      const {
        query,
        payload: body
      } = req;
      ...
      try {
        ...
        throw (0, _handle_es_error.handleESError)(error);
      }
    })
  });
}