...
exports.createInstallRoute = void 0;
...
function insertDataIntoIndex(dataIndexConfig, index, nowReference, request, server, callWithRequest) {
  const bulkInsert = async docs => {
    ...
    const resp = await callWithRequest(request, 'bulk', {
      body: bulk
    });
...
const createInstallRoute = () => ({
  path: '/api/sample_data/{id}',
  method: 'POST',
  config: {
    validate: {
      query: _joi.default.object().keys({
        now: _joi.default.date().iso()
      }),
    ...
    handler: async (request, h) => {
      ...
      try {
        createResults = await request.getSavedObjectsClient().bulkCreate(sampleDataset.savedObjects, {
          overwrite: true
        });
      } catch (err) {
        server.log(['warning'], `bulkCreate failed, error: ${err.message}`);
        return _boom.default.badImplementation(`Unable to load kibana saved objects, see kibana logs for details`);
      }
      ...
      (0, _usage.usage)(request).addInstall(params.id);
      return h.response({
