...
function FieldFormat(params) {
  ...
  this.type = this.constructor; // keep the params and defaults separate

  this._params = params || {}; // one content type, so assume text

  if (_lodash.default.isFunction(this._convert)) {
    this._convert = {
      text: this._convert
    };
  }
...
FieldFormat.prototype.params = function () {
  return _lodash.default.cloneDeep(_lodash.default.defaults({}, this._params, this.getParamDefaults()));
};
/**
...
FieldFormat.prototype.toJSON = function () {
  ...
  let params = _lodash.default.transform(this._params, function (uniqParams, val, param) {
    if (val !== defaults[param]) {
      uniqParams[param] = val;
    }
  }, {});

  if (!_lodash.default.size(params)) {
    params = undefined;
  }
