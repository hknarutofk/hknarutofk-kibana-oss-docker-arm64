...
function uiRenderMixin(kbnServer, server, config) {
  ...
  server.exposeStaticDir('/node_modules/@elastic/charts/dist/{path*}', (0, _utils.fromRoot)('node_modules/@elastic/charts/dist'));
  ...
  async function renderApp({
    ...
    const uiPlugins = [...kbnServer.newPlatform.setup.core.plugins.uiPlugins.public.entries()].map(([id, plugin]) => ({
      id,
      plugin
    }));
    ...
    const csp = (0, _csp.createCSPRuleString)(config.get('csp.rules'));
