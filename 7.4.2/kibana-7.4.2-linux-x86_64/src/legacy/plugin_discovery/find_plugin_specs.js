...
function bufferAllResults(observable) {
  return observable.pipe( // buffer all results into a single array
  (0, _operators.toArray)(), // merge the array back into the stream when complete
...
function findPluginSpecs(settings, configToMutate) {
  ...
  }).pipe((0, _operators.shareReplay)()); // find plugin packs in configured paths/dirs
  ...
  const extendConfig$ = config$.pipe((0, _operators.mergeMap)(config => pack$.pipe( // get the specs for each found plugin pack
  (0, _operators.mergeMap)(({
    pack
  }) => pack ? pack.getPluginSpecs() : []), // make sure that none of the plugin specs have conflicting ids, fail
  ...
  bufferAllResults, (0, _operators.map)(({
    spec,
    deprecations
  }) => {
