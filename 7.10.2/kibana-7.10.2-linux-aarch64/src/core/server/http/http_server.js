...
function isBoom(input) {
  return input instanceof _boom.default;
}
/** @internal */
...
class HttpServer {
  ...
  async start() {
    ...
    for (const router of this.registeredRouters) {
      for (const route of router.getRoutes()) {
        ...
        const fakeSocketTimeout = (timeout === null || timeout === void 0 ? void 0 : timeout.payload) ? timeout.payload + 1 : undefined;
        this.server.route({
          ...
          options: {
            ...
            ext: {
              onPreAuth: {
                method: (request, h) => {
                  ...
                  if (timeout === null || timeout === void 0 ? void 0 : timeout.idleSocket) {
                    ...
                    request.raw.req.socket.setTimeout((_socketTimeout = this.config.socketTimeout) !== null && _socketTimeout !== void 0 ? _socketTimeout : 0);
                  }
            ...
            validate,
            payload: [allow, maxBytes, output, parse, timeout === null || timeout === void 0 ? void 0 : timeout.payload].some(v => typeof v !== 'undefined') ? {
              allow,
              maxBytes,
              output,
              parse,
  ...
  setupKeepAliveTimeout(config) {
    const keepAliveTimeout = `timeout=${String(config.keepaliveTimeout / 1000)}`;
    this.server.ext('onPreResponse', (request, responseToolkit) => {
      var _request$headers$conn;

      const response = request.response;

      if (response && ((_request$headers$conn = request.headers.connection) === null || _request$headers$conn === void 0 ? void 0 : _request$headers$conn.includes('keep-alive'))) {
        if (isBoom(response)) {
