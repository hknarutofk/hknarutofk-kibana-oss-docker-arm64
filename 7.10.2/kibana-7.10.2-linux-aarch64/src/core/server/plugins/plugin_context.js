  ...
  return {
    ...
    config: {
      legacy: {
         ...
         * @deprecated
         */
        globalConfig$: (0, _rxjs.combineLatest)([coreContext.configService.atPath(_kibana_config.config.path), coreContext.configService.atPath(_elasticsearch_config.config.path), coreContext.configService.atPath(_utils.config.path), coreContext.configService.atPath(_saved_objects_config.savedObjectsConfig.path)]).pipe((0, _operators.map)(([kibana, elasticsearch, path, savedObjects]) => (0, _std.deepFreeze)({
