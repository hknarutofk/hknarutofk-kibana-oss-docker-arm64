"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var get_chart_container_dimensions_1 = require("./get_chart_container_dimensions");
exports.getDebugStateSelector = function (state) {
    if (state.internalChartState) {
        var _a = get_chart_container_dimensions_1.getChartContainerDimensionsSelector(state), height = _a.height, width = _a.width;
        if (height * width > 0) {
            return state.internalChartState.getDebugState(state);
        }
    }
    return {};
};
//# sourceMappingURL=get_debug_state.js.map